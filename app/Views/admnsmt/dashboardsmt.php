                ...
                <div class="card">
                    <div class="card-header">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <button id="downloadChart" class="btn btn-default btn-sma float-right">Download Grafik</button>
                        </div>
                    </div>
                    <div class="card-body-sr">
                        <div class="chart-container mt-2">
                            <form method="get" action="<?= site_url('admnsmt/dashboardsmt') ?>" class="form-container">
                                <div class="form-row">
                                    ...
                                    <div class="col-md-2">
                                        ...
                                        <select  id="tipe_ng" name="tipe_ng" class="form-control" style="margin-right: 28px; font-size: 14px">
                                            <option value="">All Data Tipe NG</option>
                                        </select>
                                    </div>
                            ...
                            <table id="solder_paste_table" class="table table-bordered table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="sortable" data-column="part_number">Part<span class="sort-icon"></span></th>
...
<script>
    $(document).ready(function() {
        ...
        const hasFilters = <?= json_encode(!empty($filters['model']) || !empty($filters['mesin']) || !empty($filters['tipe_ng'])) ?>;
        const datasets = {};

        data.forEach(item => {
            const key = hasFilters ? `${item.model}-${item.mesin}` : item.model;

            if (!datasets[key]) {
                datasets[key] = {
                    label: hasFilters ? `${item.model} - ${item.mesin}` : item.model,
        ...
        const suggestedMax = hasFilters ? 15 : 20;
        ...
        $('#resetFilters').click(function() {
            window.location.href = '<?= site_url('admnsmt/dashboardsmt') ?>';
        });
    });
</script>
...
<script>
    $(document).ready(function() {
        $('#mesin').change(function() {
            var mesin = $(this).val();
            var tipeNgSelect = $('#tipe_ng');

            tipeNgSelect.empty();
            tipeNgSelect.append('<option value="">All Tipe NG</option>');

            if (mesin) {
                $.ajax({
                    url: '<?= base_url('user/getTipeNgByMesin') ?>/' + mesin,
                    type: 'GET',
                    dataType: 'json',
                    success: function(data) {
                        $.each(data, function(index, value) {
                            tipeNgSelect.append('<option value="' + value + '">' + value + '</option>');
                        });
                    },
                    error: function(xhr, status, error) {
                        console.error('Error fetching Tipe NG data: ', error);
                    }
                });
            }
        });

        $('#mesin').trigger('change');
    });
</script>
...
<script>
    $(document).ready(function() {
        $('#line').change(function() {
            ...
            modelSelect.empty();
            modelSelect.append('<option value="">All Data Model</option>');

            if (line) {
                $.ajax({
                    url: '<?= base_url('user/getModelByLine') ?>/' + line,
                    type: 'GET',
                    dataType: 'json',
                    success: function(data) {
                        $.each(data, function(index, value) {
